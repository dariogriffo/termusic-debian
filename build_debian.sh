...
# Function to map Debian architecture to termusic release name
...
build_architecture() {
    ...
    # Build packages for Debian distributions
    ...
    for dist in "${arr[@]}"; do
        ...
        if ! docker build . -t "termusic-$dist-$build_arch" \
        ...
        id="$(docker create "termusic-$dist-$build_arch")"
